...
contract('NodeOperatorsRegistry', ([appManager, voting, user1, user2, user3, nobody]) => {
  let appBase, app, pool, steth, dao, stakingModule
  ...
  before('deploy base app', async () => {
    ...
    // inside the testing_provideDepositsData() method
  ...
  describe('finalizeUpgrade_v2()', () => {
    ...
    it('increases keysOpIndex & changes validatorKeysNonce', async () => {
      const [keysOpIndexBefore, validatorsKeysNonceBefore] = await Promise.all([app.getKeysOpIndex(), app.getDepositsDataNonce()])
      await app.finalizeUpgrade_v2(steth.address, CURATED_TYPE)
      const [keysOpIndexAfter, validatorsKeysNonceAfter] = await Promise.all([app.getKeysOpIndex(), app.getDepositsDataNonce()])
    ...
    it('emits KeysOpIndexSet & DepositsDataNonceChanged', async () => {
      const keysOpIndexBefore = await app.getKeysOpIndex()
      const receipt = await app.finalizeUpgrade_v2(steth.address, CURATED_TYPE)
      const validatorsKeysNonceAfter = await app.getDepositsDataNonce()
      assert.emits(receipt, 'KeysOpIndexSet', { keysOpIndex: keysOpIndexBefore.toNumber() + 1 })
      assert.emits(receipt, 'DepositsDataNonceChanged', { depositsDataNonce: validatorsKeysNonceAfter })
    })
  })
  ...
  describe('activateNodeOperator()', () => {
    beforeEach(async () => {
      ...
      assert.equals(validatorsReport.totalVetted, 16)
    })
  ...
  describe('deactivateNodeOperator()', async () => {
    ...
    it('resets vettedSigningKeysCount to depositedSigningKeysCount when vettedSigningKeysCount > depositedSigningKeysCount', async () => {
      ...
      assert.isTrue(
        +operatorReportBefore.totalVetted > +operatorReportBefore.totalDeposited,
      ...
      assert.equals(+operatorReportAfter.totalVetted, +operatorReportAfter.totalDeposited)

      assert.equals(
        +allValidatorsReportBefore.totalVetted - +allValidatorsReportAfter.totalVetted,
        +operatorReportBefore.totalVetted - +operatorReportAfter.totalVetted
      )
    })
  ...
  describe('setNodeOperatorStakingLimit()', async () => {
    ...
    it('emits KeysOpIndexSet & DepositsDataNonceChanged on vettedSigningKeysCount change', async () => {
      const keysOpIndexBefore = await app.getKeysOpIndex()
  ...
  describe('updateExitedValidatorsCount()', () => {
    ...
    it('reverts with "NODE_OPERATOR_NOT_FOUND" error when called on non existent validator', async () => {
      ...
      assert.isTrue(hasPermission)
      await assert.reverts(app.updateExitedValidatorsCount(notExistedNodeOperatorId, 40, { from: voting }), 'NODE_OPERATOR_NOT_FOUND')
    })
  ...
  describe('invalidateReadyToDepositValidators()', () => {
    beforeEach(async () => {
      await nodeOperators.addNodeOperator(app, NODE_OPERATORS[0], { from: voting })
      await nodeOperators.addNodeOperator(app, NODE_OPERATORS[1], { from: voting })
    ...
    it('reverts with "APP_AUTH_FAILED" error when called by sender without INVALIDATE_READY_TO_DEPOSIT_KEYS_ROLE role', async () => {
      ...
      assert.isFalse(hasPermission)
      await assert.reverts(app.invalidateReadyToDepositValidators(), 'APP_AUTH_FAILED')
    })
    ...
    it('sets totalSigningKeysCount and vettedSigningKeysCount equal to depositedSigningKeys for all node operators', async () => {
      ...
      await app.invalidateReadyToDepositValidators({ from: voting })
    ...
    it('emits TotalSigningKeysCountChanged & VettedSigningKeysCountChanged events for node operator only if it had unused keys', async () => {
      const allNodeOperatorsBefore = await nodeOperators.getAllNodeOperators(app)
      const receipt = await app.invalidateReadyToDepositValidators({ from: voting })
      const allNodeOperatorsAfter = await nodeOperators.getAllNodeOperators(app)
      for (let i = 0; i < allNodeOperatorsBefore.length; ++i) {
        const nodeOperatorBefore = allNodeOperatorsBefore[i]
        const nodeOperatorAfter = allNodeOperatorsAfter[i]
    ...
    it("doesn't change validators keys nonce if keys weren't invalidated", async () => {
      ...
      const receipt = app.invalidateReadyToDepositValidators({ from: voting })
      const validatorsKeysNonceBefore = await app.getDepositsDataNonce()
  ...
  describe('getSigningKeysAllocationData()', async () => {
    ...
    it('excludes from result node operators without unused keys', async () => {
      ...
      const availableKeysCount =
        +secondNodeOperatorReport.totalVetted - +secondNodeOperatorReport.totalDeposited
    ...
    it('allocates deposits firstly to node operators with min active keys count & with available keys', async () => {
      ...
      const firstNodeOperatorAvailableValidators =
        firstNodeOperatorReport.totalVetted - firstNodeOperatorReport.totalDeposited
      const secondNodeOperatorAvailableValidators =
        secondNodeOperatorReport.totalVetted - secondNodeOperatorReport.totalDeposited
    ...
    it("doesn't allocates keys to deactivated node operators", async () => {
      ...
      assert.equals(firstNodeOperatorReport.totalExited, 1)
      assert.equals(firstNodeOperatorReport.totalDeposited, 5)
      assert.equals(firstNodeOperatorReport.totalVetted, 5)
  ...
  describe('provideDepositsData()', async () => {
    const firstNodeOperatorId = 0
    const secondNodeOperatorId = 1
    ...
    it('reverts with error "APP_AUTH_FAILED" when called by sender without REQUEST_VALIDATORS_KEYS_FOR_DEPOSITS_ROLE', async () => {
      ...
      assert.isFalse(hasPermission)
      await assert.reverts(app.provideDepositsData(10, '0x', { from: nobody }), 'APP_AUTH_FAILED')
    })
    ...
    it('returns empty result when no validators to deposit to', async () => {
      ...
      assert.equals(nodeOperatorsCount, 0)
      const keysToAllocate = 10
      const receipt = await app.testing_provideDepositsData(keysToAllocate)
    ...
    it("doesn't change validators keys nonce when no available keys for deposit", async () => {
      ...
      const validatorsKeysNonceBefore = await app.getDepositsDataNonce()
      const keysToAllocate = 10
      await app.testing_provideDepositsData(keysToAllocate)
      const validatorsKeysNonceAfter = await app.getDepositsDataNonce()
      assert.equals(validatorsKeysNonceBefore, validatorsKeysNonceAfter)
    })
    ...
    it('loads correct signing keys', async () => {
      ...
      let receipt = await app.testing_provideDepositsData(keysToAllocate)
      ...
      keysToAllocate = 2
      receipt = await app.testing_provideDepositsData(keysToAllocate)
