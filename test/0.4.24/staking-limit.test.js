...
contract('StakingLimits', ([account1]) => {
  ...
  it('check staking rate limit', async () => {
    ...
    assert.equal(limited2, true, 'limits are not limited')
  })
  ...
  it('stake limit increase > max stake', async () => {
    ...
    const slot = await limits.setStorageStakeLimitStruct(0, 0, 0, 0)
    await limits.resumeStakingWithNewLimit(slot, maxStakeLimit, maxStakeLimitIncreasePerBlock)
    ...
    assertRevert(limits.resumeStakingWithNewLimit(slot, maxStakeLimit, maxStakeLimitGrowthBlocks), 'TOO_LARGE_LIMIT_INCREASE')
  })
  ...
  it('stake limit reverts on large values', async () => {
    ...
    const slot = await limits.setStorageStakeLimitStruct(0, 0, 0, 0)
    assertRevert(limits.resumeStakingWithNewLimit(slot, maxStakeLimit, maxStakeLimitIncreasePerBlock), 'TOO_LARGE_MAX_STAKE_LIMIT')

    maxStakeLimit = toBN(2).mul(toBN(10).pow(toBN(18)))
    maxStakeLimitIncreasePerBlock = toBN(10)
    assertRevert(limits.resumeStakingWithNewLimit(slot, maxStakeLimit, maxStakeLimitIncreasePerBlock), `TOO_SMALL_LIMIT_INCREASE`)
  })
