...
contract('Lido', ([appManager, voting, user1, user2, user3, nobody, depositor, treasury]) => {
  ...
  context('EL Rewards', async () => {
    ...
    beforeEach('set up rewarder and limits', async () => {
      ...
      const maxPositiveRebase = bn(1).mul(bn(10).pow(bn(8))) // 10%
      await assertRevert(app.setMaxPositiveRebase(maxPositiveRebase), 'APP_AUTH_FAILED')
      receipt = await app.setMaxPositiveRebase(maxPositiveRebase, { from: voting })
      assertEvent(receipt, 'MaxPositiveRebaseSet', { expectedArgs: { maxPositiveRebase: maxPositiveRebase } })
    })
    ...
    it('Attempt to set invalid execution layer rewards withdrawal limit', async () => {
      const initialValue = await app.getMaxPositiveRebase()

      assertEvent(await app.setMaxPositiveRebase(1, { from: voting }), 'MaxPositiveRebaseSet', {
        expectedArgs: { maxPositiveRebase: 1 }
      })
