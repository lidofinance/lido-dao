...
//   describe('Allocate to best candidate', () => {
//     describe('No bins', () => {
//       const amountsOfResources = [0, 1, 2, 64]
//       for (const resources of amountsOfResources) {
//         const binLoads = []
//         const binCapacities = []

//         it(`should not allocate anything with ${resources} resources`, async () => {
//           const { allocated, newAllocations } = await minFirstAllocationStrategy.allocateToBestCandidate(binLoads, binCapacities, resources)

//           assertBn(allocated, 0, 'No resources should be allocated')
//           assert.equal(newAllocations.length, 0, 'No bins')
//         })
//       }
//     })
...
//       for (const binLoads of binLoadCases) {
//         const binCapacityCases = [
//           [0],
//           [1],
//           [2],
//           [64]
...
//         for (const binCapacities of binCapacityCases) {
//           const resourcesCases = [0, 1, 2, 3, 63, 64, 65]

//           for (const resources of resourcesCases) {
//             it(`should allocate as much resources as fits into the bin or until runs out of resources
//               binLoads=${binLoads}, binCapacities=${binCapacities}, resources=${resources}
//             `, async () => {
//               const { allocated, newAllocations } =
//                 await minFirstAllocationStrategy.allocateToBestCandidate(binLoads, binCapacities, resources)
//               assertBn(allocated, Math.min(resources, Math.max(0, sum(binCapacities) - sum(binLoads))))
//               assert(newAllocations[0], binLoads[0] + allocated)
//               assert.equal(newAllocations.length, 1)
//             })
//           }
//         }
