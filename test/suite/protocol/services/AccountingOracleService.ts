...
import { ONE_DAY, SHARE_RATE_PRECISION, ZERO_BYTES32 } from "../constants";
import { LidoProtocol } from "../types";

interface OracleReportOptions {
  clDiff: bigint;
...
interface PushOracleReportOptions {
  refSlot: bigint;
  clBalance: bigint;
...
export class AccountingOracleService {
  constructor(protected readonly protocol: LidoProtocol) {}

  /**
   * Fast forwards time to next report, compiles report, pushes through consensus to AccountingOracle.
   */
  async oracleReport({
    clDiff = ether("10"),
  ...
  }: Partial<OracleReportOptions> = {}) {
    const { hashConsensus, lido, elRewardsVault, withdrawalVault, burner, accountingOracle } = this.protocol.contracts;

    // Fast-forward to next report time
    if (waitNextReportTime) {
      await this.waitNextAvailableReportTime();
    }
    ...
    if (!skipWithdrawals) {
      const simulatedReport = await this.simulateReport(
        refSlot,
      ...
      if (withdrawalFinalizationBatches.length === 0) {
        withdrawalFinalizationBatches = await this.getFinalizationBatches(simulatedShareRate, withdrawals, elRewards);
      }
  ...
  async pushOracleReport({
    ...
    extraDataItemsCount = 0n,
    extraDataList = new Uint8Array(),
  }: PushOracleReportOptions) {
    const { accountingOracle } = this.protocol.contracts;
    ...
    const { report, hash } = this.prepareOracleReport({
      consensusVersion,
      refSlot,
    ...
    const submitter = await this.reachConsensus(refSlot, hash, consensusVersion);

    await setBalance(submitter.address, ether("100"));
  ...
  private async reachConsensus(refSlot: bigint, reportHash: string, consensusVersion: bigint) {
    const { hashConsensus } = this.protocol.contracts;
  ...
  private async waitNextAvailableReportTime(): Promise<void> {
    const { hashConsensus } = this.protocol.contracts;
  ...
  /**
   * Simulates the report for the given ref. slot and returns the simulated share rate.
   */
  private async simulateReport(
    refSlot: bigint,
    ...
    | undefined
  > {
    const { hashConsensus, accountingOracle, lido } = this.protocol.contracts;
    ...
    try {
      log.debug("Simulating oracle report", {
        ...
        "Post CL Balance": ethers.formatEther(postCLBalance),
  ...
  private async getFinalizationBatches(
    shareRate: bigint,
    limitedWithdrawalVaultBalance: bigint,
    limitedElRewardsVaultBalance: bigint,
  ): Promise<bigint[]> {
    const { oracleReportSanityChecker, lido, withdrawalQueue } = this.protocol.contracts;
    ...
    // hack to avoid internal Result[] => bigint[] conversion
  ...
  private prepareOracleReport(report: AccountingOracle.ReportDataStruct) {
    const items = this.getReportDataItems(report);
    const hash = this.calcReportDataHash(items);
    return { report, items, hash };
  }

  private getReportDataItems(report: AccountingOracle.ReportDataStruct) {
    return [
  ...
  private calcReportDataHash(items: ReturnType<typeof this.getReportDataItems>): string {
    const types = [
