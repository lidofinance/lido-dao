...
const { pad, ETH, hexConcat, toBN } = require('../helpers/utils')
...
contract('SepoliaDepositAdapter impl', ([deployer]) => {
  ...
  before('deploy lido with dao', async () => {
    ...
    // await depositAdapter.waitForDeployment();
  
    
    const testVersion = await depositAdapter.VERSION()
    assert.equals(testVersion, 2)

    let depositCaller = depositAdapter.address;
    ...
    // await impersonatedSigner.sendTransaction({ to: depositCaller, value: ethers.utils.parseEther('5.0') })
    ...
    // const withdrawalCredentials = deployer
    const withdrawalCredentials = '0x005bfe00d82068a0c2a6687afaf969dad5a9c663cb492815a65d203885aaf993'
    const sig = '0x802899068eb4b37c95d46869947cac42b9c65b90fcb3fde3854c93ad5737800c01e9c82e174c8ed5cc18210bd60a94ea0082a850817b1dddd4096059b6846417b05094c59d3dd7f4028ed9dff395755f9905a88015b0ed200a7ec1ed60c24922'
