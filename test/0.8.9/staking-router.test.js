...
contract('StakingRouter', (accounts) => {
    ...
    var stranger2 = accounts[5]
     ...
     beforeEach(async () => {
        ...
        stakingRouter = await StakingRouter.new(lido.address, { from: appManager })
     ...
     describe('staking router test', () => {
        ...
        it(`init counters and burn amount per run works`, async () => {
            ...
            for(i=0; i< modules.length; i++) {
                ...
                let name = ModuleTypes[module.type] + i

                await stakingRouter.addStakingModule(name, _module.address, { from: appManager })
                ...
                await _module.setSoftCap(module.softCap, { from: appManager })
            ...
            let b1 = await lido.balanceOf(externalAddress)
            ...
            let strangerBalance = await lido.balanceOf(externalAddress)
            console.log('strangerBalance', strangerBalance.toString())

            let stranger2Balance = await lido.balanceOf(stranger2)
            console.log('stranger2Balance', stranger2Balance.toString())

            for(i=0; i < modules.length; i++) {
                let module = modules[i]
                let balance = await lido.balanceOf(module.address)
            ...
            console.time()
            let deposits = await stakingRouter.getModuleDeposits(3)
            console.timeEnd()
            ...
            let op1 = await ModulePro.at(modules[0].address)
            let op1totalKeys = await op1.totalKeys()
            ...
            let tx2 = await op1.addNodeOperator('fo o', ADDRESS_1, { from: appManager })
            ...
            let tx3 = await stakingRouter.eth2deposit(1000)
            ...
            // deposits = await stakingRouter.getModuleDeposits(3)
            // console.timeEnd()
