// This spec is optimized for Gambit run
// Omitted rules that:
// - take more than 300s (seconds) to run
// - fail
 ...
 **************************************************/
methods {
    function _.submitConsensusReport(bytes32 report, uint256 refSlot, uint256 deadline) internal => CONSTANT;
    function _.getLastProcessingRefSlot() internal => CONSTANT;
    function _.getConsensusVersion() internal => CONSTANT;

    function _.reportReceivedCounter(uint256 refSlot, address member, bytes32 report) internal => getSuccessfulReportSubmitEventInfo(refSlot, member, report) expect uint256 ALL;
}
...
ghost bytes32 ghostReportHash {
    init_state axiom ghostReportHash == to_bytes32(0);
}
...
function saneTimeConfig() returns uint256 {         // returns the timestamp when the valid setting are set
    ...
    require to_mathint(initialEpoch) < ((e0.block.timestamp - genesisTime) / (secondsPerSlot * slotsPerEpoch)); // sane configuration of initialEpoch
...
definition ZERO_HASH() returns bytes32 = to_bytes32(0); // bytes32(0)
...
// Status: Pass (with simplification) - 127s
    ...
    filtered { f -> !f.isView }
{
    ...
    bool hasRole_MANAGE_FRAME_CONFIG_ROLE       = hasRole(e,MANAGE_FRAME_CONFIG_ROLE(e),e.msg.sender);
    bool hasRole_MANAGE_FAST_LANE_CONFIG_ROLE   = hasRole(e,MANAGE_FAST_LANE_CONFIG_ROLE(e),e.msg.sender);
    bool hasRole_MANAGE_MEMBERS_AND_QUORUM_ROLE = hasRole(e,MANAGE_MEMBERS_AND_QUORUM_ROLE(e),e.msg.sender);
    bool hasRole_DISABLE_CONSENSUS_ROLE         = hasRole(e,DISABLE_CONSENSUS_ROLE(e),e.msg.sender);
    bool hasRole_MANAGE_REPORT_PROCESSOR_ROLE   = hasRole(e,MANAGE_REPORT_PROCESSOR_ROLE(e),e.msg.sender);
    bool hasRole_DEFAULT_ADMIN_ROLE             = hasRole(e,DEFAULT_ADMIN_ROLE(e),e.msg.sender);
    ...
    assert ((f.selector == sig:setFrameConfig(uint256,uint256).selector) &&
            !hasRole_MANAGE_FRAME_CONFIG_ROLE) => callReverted;
    
    assert ((f.selector == sig:setFastLaneLengthSlots(uint256).selector) &&
            !hasRole_MANAGE_FAST_LANE_CONFIG_ROLE) => callReverted;
    
    assert ((f.selector == sig:addMember(address,uint256).selector) &&
            !hasRole_MANAGE_MEMBERS_AND_QUORUM_ROLE) => callReverted;
    
    assert ((f.selector == sig:removeMember(address,uint256).selector) &&
            !hasRole_MANAGE_MEMBERS_AND_QUORUM_ROLE) => callReverted;

    assert ((f.selector == sig:setQuorum(uint256).selector) &&
    ...
    assert ((f.selector == sig:disableConsensus().selector) && 
    ...
    assert ((f.selector == sig:setReportProcessor(address).selector) &&
            !hasRole_MANAGE_REPORT_PROCESSOR_ROLE) => callReverted;
    
    assert ((f.selector == sig:submitReport(uint256,bytes32,uint256).selector) &&
            !isMember) => callReverted;

    assert ((f.selector == sig:updateInitialEpoch(uint256).selector) &&
...
//  3. setFrameConfig() updates epochsPerFrame in a way that either keeps the current reference slot
//     the same or increases it by at least the minimum of old and new frame sizes
// Status: Pass (only first two asserts) - 132s
...
rule addMemberCorrectness() {
    ...
    assert lengthOf_memberAddressesAfter == require_uint256(lengthOf_memberAddressesBefore + 1); // total members increases correctly
    assert lengthOf_memberStatesAfter == require_uint256(lengthOf_memberStatesBefore + 1);       // total members increases correctly
}
...
rule removeMemberADoesNotModifyMemberB() {
    ...
    if (!isUserBMemberBefore) {
        address user = get_memberAddresses(e,require_uint256(lengthOf_memberAddressesBefore - 1));
...
rule removeMemberCorrectness() {
    ...
    assert lengthOf_memberAddressesAfter == require_uint256(lengthOf_memberAddressesBefore - 1); // total members decreases correctly
    assert lengthOf_memberStatesAfter == require_uint256(lengthOf_memberStatesBefore - 1);       // total members decreases correctly
}
...
rule setQuorumCorrectness() {
    ...
    assert (quorum <= require_uint256(lengthOf_memberAddresses / 2)) => callReverted;
...
rule sameMemberCannotSubmitDifferentReportForTheSameSlot() {
    ...
    require slot > require_uint256(lastConsensusRefSlot);  // reporting for a slot without consensus
    ...
    assert (ghostReportHash != to_bytes32(0)) && (e2.msg.sender == e.msg.sender) && (report != report2) => lastReverted;
}
...
rule variantsResetUponNewFrameStart() {
    ...
    uint256 slot; bytes32 report; uint256 consensusVersion;
    require slot > lastProcessingRefSlot;  // otherwise the report doesn't matter
    require slot > assert_uint256(lastConsensusRefSlot);  // reporting for a slot without consensus
    
    submitReport(e, slot, report, consensusVersion);

    uint256 reportVariantsLength = getReportVariantsLength(e2);

    assert ((ghostReportHash != to_bytes32(0)) && (assert_uint256(lastReportRefSlot) != slot)) => reportVariantsLength == 1;
}
...
//     filtered { f -> !f.isView && 
//                     f.selector != sig:updateInitialEpoch(uint256).selector } // this should revert once initialized (see rule 26)
// {
//     env e; calldataarg args; env e2;
//     require e.block.timestamp > saneTimeConfig();       // time moves forward (saneTimeConfig is for correct initializing)
...
// // Status: Fails (as expected, no issues) - 393s
