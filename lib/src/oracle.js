...
const LegacyOracle = getContract('LegacyOracle')

async function getOracle(web3, address) {
  LegacyOracle.setProvider(web3.currentProvider)
  return await LegacyOracle.at(address)
}
...
async function proposeBeaconSpecChange(oracle, voting, tokenManager, newSpec, txOpts = {}) {
  ...
  const calldata = await oracle.contract.methods
    .setBeaconSpec(
      updatedSpec.epochsPerFrame,
      updatedSpec.slotsPerEpoch,
      updatedSpec.secondsPerSlot,
      updatedSpec.genesisTime
    )
    .encodeABI()
