...
contract LidoOracle is ILidoOracle, AragonApp {
     ...
     */
    function _findQuorumValue(uint256 _quorum, uint256[] _data) internal pure returns (bool isQuorum, uint256 quorumValue) {
        assert(_quorum > 0);
        assert(_data.length >= _quorum);
        if (_quorum == 1) {
            assert(_data.length == _quorum);
        }
        
        // allocate arrays
        uint256[] memory dataValues = new uint256[](_data.length);
        uint256[] memory dataValuesCounts = new uint256[](_data.length);
        ...
        for (i = 0; i < _data.length; i++) {
            ...
            if (complete) {
                dataValues[dataValuesLength] = _data[i];
                dataValuesCounts[dataValuesLength]++;
                dataValuesLength++;
            }
            if (dataValuesCounts[j] == _quorum) {
                quorumValue = dataValues[j];
                return (true, quorumValue);
            }
        }

        assert(dataValuesLength <= _data.length);
