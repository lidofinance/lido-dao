...
contract AccountingOracle is BaseOracle {
    ...
    struct ReportData {
        ...
        /// @dev How much ETH is required to lock on the WithdrawalQueue contract in order to
        /// finalize requests up to id lastFinalizableWithdrawalRequestId using the max share
        /// rate simulatedShareRate. Must be set to zero if lastFinalizableWithdrawalRequestId
        /// is zero.
        uint256 etherToLockOnWithdrawalQueue;

        /// @dev The share/ETH rate with the 10^27 precision (i.e. the price of one stETH share
        /// in ETH where one ETH is denominated as 10^27) that would be effective as the result
        /// of applying this oracle report at the reference slot, with simulatedShareRate and
        /// lastFinalizableWithdrawalRequestId both set to 0.
